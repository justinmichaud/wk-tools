...
# make exits 0 on a tree with nothing left to do, so the named image's mtime
# must be at least as new as this build's own start (portable stat: GNU's -c,
# BSD/macOS's -f).
...
# (targets/container.sh), not as flags: a host path would name nothing inside
# this container. Their absence means the mount is missing, so this fails
# rather than building where `wk rm` would discard the downloads.
...
# The tree: every WPE configuration here pins to 2020.02, since the release-pinned cog
# defconfigs only make sense against it. An output/ tree is tens of gigabytes,
# so a second run at the pin continues rather than re-cloning.
...
# This repository's corrections to the vendor tree: the fork pins package versions it never built (wpebackend-fdo 1.14 is a meson
# project; the fork's package still calls cmake). Applied as patches so a fresh
# clone converges to the same tree; an applied patch is skipped.
...
# package, and takes a space-separated list. Neither script carries a
# credential: the tailnet key and wifi passphrase arrive with the card.
OVERLAY=""
...
# The pinned kernel, declared by the profile, not built here. One Debian package carries the whole
# set -- kernel, modules, device trees, overlays -- so the four cannot disagree
# about a version. Unpacked once and read by two consumers below: an overlay
# for the modules, and a post-image script for the boot files.
KERNEL_STAGE=""
if [ -n "$KERNEL_TAR" ]; then
    ...
    # The modules arrive decompressed with modules.dep already rebuilt
...
# The pinned kernel's boot files go in through buildroot's own post-image hook,
# which runs after the root filesystem is built and before the board script
# assembles the card.
...
# survives buildroot's recursive makes.
{
    ...
    # Installed by a post-image script that runs before the board's own, which
    # assembles the card image, so genimage picks these up rather than the ones
    # buildroot built. The board's script keeps its arguments; ours takes none.
    ...
    # read it (BR2_JLEVEL; its default is nproc+1, five times what the guard
    # budgets at 2 GB/job). An environment BR2_JLEVEL is inert: kconfig symbols
    # come from .config.
...
# The linker flag above reaches cmake packages through the exported toolchain
# file, written once at toolchain install; buildroot's own reinstall rewrites
# one that predates the flag.
...
# (image/buildroot-webkit.sh) leaves a local.mk pointing the wpewebkit package
# at the workspace checkout, so that override is dropped and the package
...
# several 2009-era configure scripts refuse to run as root. Start time taken
# here, not at the top: cloning and pinning can take minutes and touch nothing
# under output/.
build_start=$(date +%s)
