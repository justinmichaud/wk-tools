#!/usr/bin/env python3
"""One digest over a directory's file names, contents, symlinks and exec bits -- the exec bit is content, since a `wk` that arrives unexecutable is a tree that looks right and runs nothing -- so a tree copied onto a machine can be compared with the tree it came from; travels on stdin, so the far side needs no copy of wk-tools to run it."""
import argparse
import hashlib
import os
import sys
