...
#             libraries. Reserved, not implemented -- see
...
# Pinned: `wkdev-create --arch` would otherwise resolve to the aarch64 image
# and hand podman that with --arch=arm. 24.04_arm32_arm64 is not a substitute
# despite the name -- an arm64 image with armhf as a foreign multiarch arch.
...
# podman's name for 32-bit ARM is "arm"; ours is armhf since "arm" is ambiguous.
arch_podman() {
    case "$1" in
        armhf) echo arm ;;
        *)     echo "" ;;
...
# No for armhf: NVIDIA's userspace libraries are aarch64-only (host/linux/gpu.sh).
...
# Prepended, never replacing. Pins an FPU (JSC's ARMv7 JIT requires VFP) and
# Thumb-2. -Wno-pass-failed: WTF's vectorize pragmas fail on ARMv7 regardless
# of -mfpu, not fixable here. No -mno-unaligned-access: JSC crashes regardless
# (unaligned `ldmia` in OpEnumeratorPutByVal::decode) -- a WebKit bug, not a
# flag to compensate for. See docs/HANDOFF-linux-arm32.md.
arch_cflags() {
    case "$1" in
...
# arch_cmake <arch> [port] -- port distinguishes WPE/GTK-only USE_VULKAN from
# a JSCOnly build, where naming it would warn unused. USE_LD_LLD=OFF: WebKit
# probes for lld and appends its own -fuse-ld=lld, and the last flag wins over
# -fuse-ld=gold above. No attempt to force the JIT on: trunk has no ARMv7
# assembler (`PlatformEnable.h:724-727` undefines ENABLE_JIT for
# `#if !CPU(ADDRESS64)`). See docs/HANDOFF-linux-arm32.md.
arch_cmake() {
    local flags=""
    case "$1" in
        armhf)
            flags="-DUSE_LD_LLD=OFF"
            # Off: the arm32 image predates these deps (no volk, no `vpx`
            # target, broken Qt6 path). Delete with a newer image.
            case "${2:-}" in
                --wpe|--gtk)
