...
- [ ] audit the widened API path: `api.github.com:443` is allowed and its TLS terminated by the injector, which is the only interception in the design. Reads are authenticated from a standing token in either switch position; every write is authenticated while push is on and refused by the injector (412, naming the switch) while it is off (`container/proxy/github-inject.py`). `bugs.webkit.org:443` is the second terminated host: its `api_key` is the switch's alone, so with push off every Bugzilla read is anonymous and every write refused. What is owed is whether the leaf key living permanently on the machine is the right trade, and what a workspace can reach with a read of any private repository the token can see, or with a Bugzilla key that is the whole account [decision]
- [ ] a build box now holds no deploy key at rest (`remote/provision.sh` writes an ssh config with no `IdentityFile`; the key is a forwarded agent's). Owed: wire the forwarding into the push path so an explicit push from a workstation forwards its `wk-ssh-agent` to the box while a build or agent session forwards none, and `wk push status --target <box>` reports that arrangement instead of exit 5. Until then a push from a build box needs the person to forward the agent by hand (`ssh -A`, SSH_AUTH_SOCK pointed at the wk agent)
