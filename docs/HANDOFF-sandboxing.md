...
- [ ] audit the widened API path: `api.github.com:443` is allowed and its TLS terminated by the injector, which is the only interception in the design. The method and path are restricted (`ALLOW`, `container/proxy/github-inject.py`); what is owed is whether the leaf key living permanently on the machine is the right trade, and whether the wildcard owner and repo in every row should be pinned [decision]
