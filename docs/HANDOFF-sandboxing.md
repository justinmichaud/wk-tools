...
- [x] disable `git commit` inside a container while an agent holds it: `wk ai claude` runs the agent under bwrap with `.git/{objects,refs,logs,HEAD,packed-refs}` read-only (commit_wall_prefix, lib/common.sh), unprivileged and un-escapable (no in-container sudo change needed -- the container has no SYS_ADMIN, so nothing inside can mount); `wk verify` proves it blocks a commit in the container; `wk push on[ --force]` is the one off switch; tests/test_commit_wall.py
