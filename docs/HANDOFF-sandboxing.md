...
- [ ] audit the widened API path: `api.github.com:443` is allowed and its TLS terminated by the injector, which is the only interception in the design. Reads are open on every path and authenticated from a standing token; writes are the `ALLOW` table and the switch's token (`container/proxy/github-inject.py`). What is owed is whether the leaf key living permanently on the machine is the right trade, whether the wildcard owner and repo in every write row should be pinned, and what a workspace can reach with a read of any private repository the token can see [decision]
