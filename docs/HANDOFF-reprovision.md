...
- [ ] a second device: fresh clone + `./setup` picks up every registry machine as a target with no state copied from the first [needs a second device]
- [ ] confirm `./setup` killed mid-stage still converges: a re-run completes only what's missing, a second full run reports no changes [needs a machine to reprovision]
