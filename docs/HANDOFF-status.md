...
- [ ] container/broker/wk-broker.py writes its own key=value record per request (`$(wk_state_dir)/broker/<rid>/status`, read back by `in_flight` and `request_status`), which is a second record shape for a long-running job. lib/task.sh's shape lives under `$WK_STORE/task`, and on a macOS workstation that is the podman VM's store, which the host the broker runs on cannot write -- converging needs one location both halves can write [no hardware needed]
