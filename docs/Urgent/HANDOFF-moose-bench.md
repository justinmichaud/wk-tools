...
- [ ] add a bench-role conf in `machines/` that names the peer (`NODE_SSH=moose`) and `boot/uefi-bootnext.sh` (same `b_*` driver contract as `boot/rpi5-usb.sh`), dry-run only; `wk boot moose` must report that arming needs interactive sudo rather than failing under `sudo -n` [needs moose]
