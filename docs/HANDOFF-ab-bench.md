...
- [ ] boot `wpewebkit-2.38-buildroot-rpi4-32` from the rpi4's SD once: the image has never booted on hardware, and the image on moose predates `image/buildroot/fleet-overlay.sh`, so it neither parks the card nor reboots unless claimed -- a person pulls the card or re-arms the stick by hand until it is rebuilt [needs a person within reach of the board]
