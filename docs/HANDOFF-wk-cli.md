...
- [ ] `justinmichaud/WebKit`'s `main` is a protected branch (`gh api repos/justinmichaud/WebKit/branches/main` -> `protected: true`), so the one thing `container/bin/git-sync-fork` exists to do -- fast-forward the fork's `main` to origin's -- is refused by GH006, and `git-webkit pr` reports the same push as a non-fatal sync failure on every run. Either the protection comes off the fork's `main`, or the helper says so by name instead of pushing into a refusal [decision]
- [ ] `wk key check` measures the Bugzilla key's authentication (`/rest/valid_login`) and not what its account may change; the github-pat rule measures scope, this one cannot, because bugs.webkit.org discloses group membership only to a caller with editusers. Its wording no longer implies capability, but a key reports `ok` while `git-webkit pr`'s bug update is refused: assigning a bug needs editbugs or being its assignee, and bugs.webkit.org answers that refusal with HTTP 401 and error code 115, which reads as an authentication failure in any log that keeps only the status [decision]
...
- [ ] on macOS the `vm` target's store is `wk_record_dir`, which answers with `$WK_STORE` itself whenever that is not the default, so a scratch store (every test that sets one) puts the container and vm targets on one directory and `ws_target` refuses the name: "workspace '<n>' exists on targets: container vm". `tests/test_registry_free.py`'s `test_ws_target_resolves_a_workspace_in_a_fake_local_store` fails on exactly that, identically at HEAD and with the vm target dropped from the walk off macOS; `tests/test_lifecycle.py`'s `test_stop_start_shared_mount_and_multi_rm` fails with the same message and its teardown `wk rm` then refuses too, leaving the container and its overlay behind (seven such leftovers were on this Mac on 2026-09-21) -- that second instance reads the same shared directory through the creation record rather than `ws/`, and is not separately isolated. The fix is the vm target getting a store of its own, which moves where a guest's record lives [decision]
...
- [ ] audit the name `store`: `$WK_STORE` holds `ws/`, `git/WebKit.git` (the host's, mounted, on macOS), `base/`, `cache/`, `bench/`, `skills/`, `vm/`, with `secrets/`, `push-keys/` and `agent-rw/` beside it (`~/.config/wk` on a macOS host, the store itself elsewhere), and only the mirror is WebKit's -- decide what each part is called before a second project needs a name [needs a decision]
...
- [ ] `wk build --detach` on a remote target: the pre-written `state=running` can reach the far machine before its log is truncated, so a stale log from a previous run reads as `silent` -- counted busy, waited through -- for one poll [needs a remote target]
...
- [ ] `wk new <name> --target <peer>` refuses here and names the command to run over there -- every other workspace command, `wk rm` now included, is handed to or asked of the peer, so this is the one a person still types on the other machine [needs a peer]
- [ ] `wk rm <ws>` of a workspace a peer owns, against a real peer: that machine's own `wk rm` runs there with the answer given here, and its listing is read back before this end reports the workspace gone [needs a peer]
- [ ] `wk rm --all` against the real fleet: the containers in the podman VM, a macOS guest, a build box's checkout and a peer's own workspaces in one question, then the removal each one routes to [needs the fleet]
