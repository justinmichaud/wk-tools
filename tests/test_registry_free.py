...
class TestWsTargetDerivesFromTheStore(WkTest):
    ...
    def test_ws_target_resolves_a_live_creating_record_to_its_target(self):
        ...
        with tempfile.TemporaryDirectory(prefix="wk-registry-free-") as tmp:
            ...
            # No ws/<name> anywhere -- only the creation's task record, which
            # `wk new` writes as its first act into the store of the target it
            # was asked for, so the record's own location names that target.
            # Its pid is this very bash process's, alive for exactly as long as
            # the ws_target call below that has to trust it.
            cp = bash(f'''
{_SOURCES}
( export WK_STORE="{tmp}"
  . "{REPO}/lib/task.sh"
  task_begin new here {name} "wk new {name} --kill" /nolog checking >/dev/null )
WK_VM_STORE="{tmp}" ws_target {name}
''')
            self.assertEqual(cp.returncode, 0, cp.stderr)
            self.assertEqual(
                cp.stdout.strip(), "vm",
                "a live creation record in the vm store, with no store directory "
...
( export WK_STORE="{tmp}"
  . "{REPO}/lib/task.sh"
  d=$(task_begin new here {name} "wk new {name} --kill" /nolog checking)
  task_pid "$d" 4194304 )
WK_VM_STORE="{tmp}" ws_target {name}
''')
            self.assertEqual(cp.returncode, 0, cp.stderr)
