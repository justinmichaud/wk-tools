...
#
# MACH_DEVICE is the bench medium and the one the firmware tries first
# (`wk pi boot-order <machine>` writes BOOT_ORDER from it); the rescue is on
# the disk MACH_ROOT is a partition of, tried second. The rpi4 has neither the
...
# Not `$MACH_DEVICE`, a kernel name: "the SD card" survives another card
...
pimbr_dev() {
    ...
    [ -n "$_PIMBR_DEV" ] || die "cannot tell which disk on $MACH_NAME is its bench medium.
...
_pimbr_set_type() {
    ...
    firmware boots the $(_pimbr_word "$MACH_DEVICE") or steps over it to the rescue, so a write
...
b_arm() {
    local state
    state=$(pimbr_state) || die "could not read $MACH_DEVICE's partition table on $MACH_NAME.
    Arming this machine means writing one byte of it, so the bench medium has to be
    attached and readable from the rescue."

    case "$state" in
        armed)
            debug "$MACH_NAME's $(_pimbr_word "$MACH_DEVICE") is already armed"
            return 0 ;;
        disarmed)
            _pimbr_set_type "$PIMBR_TYPE_ARMED" \
                || die "could not arm the $(_pimbr_word "$MACH_DEVICE") on $MACH_NAME" ;;
        *)
            die "$MACH_DEVICE on $MACH_NAME has partition type 0x$(_pimbr_type) on partition
...
b_disarm() {
    ...
    _pimbr_set_type "$PIMBR_TYPE_DISARMED" \
        || die "could not disarm the $(_pimbr_word "$MACH_DEVICE") on $MACH_NAME"
}

b_disarm_note() {
    log "  $MACH_DEVICE's partition $PIMBR_PART_SUFFIX is typed 0x$PIMBR_TYPE_DISARMED, so the firmware finds no"
    log "  boot filesystem there and $MACH_NAME boots its rescue on $(pimbr_rescue_disk). 'wk boot $MACH_NAME' puts it back."
}
...
b_media() {
    ...
    bench=$(_pimbr_word "$MACH_DEVICE"); rescue=$(_pimbr_word "$(pimbr_rescue_disk)")
    case "${MODE:-}" in
        ...
        base*)  id=$(b_device_image 2>/dev/null || true)
                ...
                printf 'booted its rescue on the %s (%s); %s %s holds %s, %s' \
                    "$rescue" "${MODE#base }" "$bench" "$MACH_DEVICE" \
                    "${id:-no wk system (wk sysimage write puts one there)}" "${state:-unreadable}"
                return 0 ;;
        host)   ;;
        *)      printf '%s %s: state unknown (board unreachable); the %s is the rescue' "$bench" "$MACH_DEVICE" "$rescue"; return 0 ;;
    esac
    id=$(b_device_image 2>/dev/null || true)
    state=$(pimbr_state 2>/dev/null) || true
    printf '%s %s holds %s, %s; the %s is the rescue' \
        "$bench" "$MACH_DEVICE" "${id:-no wk system (wk sysimage write puts one there)}" "${state:-unreadable}" "$rescue"
}
...
wk pi boot-order $MACH_NAME
    the $(_pimbr_word "$MACH_DEVICE") first, the rescue behind it
wk sysimage write <id> --disk $MACH_NAME:$MACH_DEVICE
    the $(_pimbr_word "$MACH_DEVICE") -- the system it is measured on
wk boot $MACH_NAME
