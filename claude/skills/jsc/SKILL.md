...
- **Never put anything global in project memory — it goes in the relevant skill.** This covers every always-apply rule, convention, preference, and measurement/debugging technique, not just voice and workflow: if it would be true on another machine, in another checkout, or for the next task of its kind, it belongs in a skill. Project memory is keyed to one checkout path on one workstation, and the user works across several, so a global rule stored there silently fails to travel. Memory is only for what is genuinely local and non-derivable: this box's hardware quirks, machine-specific paths, in-flight project state, and *measured results* for a specific branch. When a session teaches you a rule, edit the skill that owns it (`jsc-jetstream-compare` for benchmarking, `jsc-profile` for profiling, this skill for anything cross-cutting) — and if you catch a global rule already sitting in memory, move it into the skill and delete it from memory.
...
**Always prefer `constexpr` to `const`.** Any declaration whose initializer is a compile-time constant gets `constexpr`: file-scope and namespace-scope constants, static data members (`static constexpr unsigned maxLength = 16;`), lookup tables (`static constexpr std::array<...>`), and local constants. That makes the compile-time evaluation a compile error if it ever stops holding, and leaves no static initializer to run at startup. Mark a helper that computes such a value `constexpr` too, so callers can use it in a constant context. Write `const` only where `constexpr` is not applicable — a local initialized from a runtime value, a `const` member function, a `const&`/`const*` parameter or return type, or a smart-pointer member that is never reassigned. When editing a line that already declares a compile-time constant as `const`, change it to `constexpr`; do not sweep unrelated lines just for this (see the no-unrelated-changes rule).
